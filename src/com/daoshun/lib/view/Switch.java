...
/**
 * A Switch is a two-state toggle switch widget that can select between two options. The user may
 * drag the "thumb" back and forth to choose the selected option, or simply tap to toggle as if it
 * were a checkbox. The {@link #setText(CharSequence) text} property controls the text displayed in
 * the label for the switch, whereas the {@link #setTextOff(CharSequence) off} and
 ...
 * {@link #setSwitchTextAppearance(android.content.Context, int) switchTextAppearance} and the
 * related seSwitchTypeface() methods control that of the thumb.
 * 
 */
public class Switch extends CompoundButton {
    ...
    public Switch(Context context, AttributeSet attrs) {
        this(context, attrs, R.style.Switch);
    }
     ...
     * @param defStyle
     *            An attribute ID within the active theme containing a reference to the default
    ...
    /**
     * Sets the typeface and style in which the text should be displayed on the switch, and turns on
     * the fake bold and italic bits in the Paint if the Typeface that you provided does not have
     ...
     * Typeface families actually have bold and italic variants, so you may need to use
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        final int switchWidth =
                Math.max(mSwitchMinWidth, maxTextWidth
                        * 2 + mThumbTextPadding * 4 + mTempRect.left + mTempRect.right);
    ...
    private boolean hitThumb(float x, float y) {
        ...
        final int thumbRight =
                thumbLeft + mThumbWidth + mTempRect.left + mTempRect.right + mTouchSlop;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        case Gravity.CENTER_VERTICAL:
            switchTop =
                    (getPaddingTop() + getHeight() - getPaddingBottom()) / 2 - mSwitchHeight / 2;
            switchBottom = switchTop + mSwitchHeight;
            break;
